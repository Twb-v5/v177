...
// من localStorage: "tawbah\_gender" = "male" | "female"
...
tawbah\_session\_id          → معرف الجلسة

tawbah\_gender              → "male" | "female"

zakiy\_voice\_profile        → معرف البروفايل الصوتي المختار

zakiy\_voice\_input          → نص صوتي معلق للإرسال

tawbah\_theme               → "light" | "dark"

tawbah\_accent              → AccentColor

tawbah\_lang                → "ar" | "en"

tawbah\_streak              → عدد أيام الالتزام

tawbah\_covenant\_date       → تاريخ العهد

tawbah\_inactive\_days       → أيام الغياب

tawbah\_risk\_score          → riskScore المحسوب

tawbah\_danger\_hours        → أوقات الخطر \[ساعات]

sos\_count                  → عدد مرات استخدام SOS

sos\_last                   → آخر استخدام SOS

sos\_return                 → علم العودة من SOS
...
OPENAI\_API\_KEY           → GPT-4o + Whisper + TTS

DATABASE\_URL             → PostgreSQL (Supabase)

SUPABASE\_URL             → Supabase URL

SUPABASE\_ANON\_KEY        → Supabase Anon Key

VAPID\_PUBLIC\_KEY         → Web Push

VAPID\_PRIVATE\_KEY        → Web Push

API\_BASE\_URL             → رابط الـ Backend من Frontend
