...
function SectionLabel({ title, isDark }: { title: string; isDark: boolean }) {
  return (
    <Text style={{
      ...
      textTransform: "uppercase", letterSpacing: 0.5, fontFamily: "IBMPlexSansArabic_700Bold",
    }}>
      {title}
    </Text>
  );
...
export default function NotificationsScreen() {
  ...
  const update = async (key: keyof NotifSettings, value: boolean | number) => {
        ...
        <View style={{ marginHorizontal: 16, backgroundColor: cardBg, borderRadius: 16, borderWidth: 1, borderColor, padding: 16 }}>
          <View style={{ flexDirection: "row-reverse", gap: 8, flexWrap: "wrap" }}>
            {ADVANCE_OPTIONS.map((min) => (
              <Pressable key={min} onPress={() => update("prayerAdvance", min)}
                style={{
                  paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10,
                  backgroundColor: settings.prayerAdvance === min ? accentColor : (isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"),
                  borderWidth: 1, borderColor: settings.prayerAdvance === min ? accentColor : borderColor,
                }}>
