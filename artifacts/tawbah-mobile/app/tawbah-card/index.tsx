...
import {
  View, Text, ScrollView, Pressable, TextInput, Share,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { useColors } from "@/hooks/useColors";
import { PageHeader } from "@/components/shared/PageHeader";
import { ImageIcon, Share2, RefreshCw, Check, Eye, EyeOff } from "lucide-react-native";
...
function CardPreview({ theme, verse, name, showName, dayNumber, hijriDate }: {
  theme: typeof THEMES[0]; verse: typeof VERSES[0];
  name: string; showName: boolean; dayNumber: number; hijriDate: string;
}) {
  return (
    <View style={{
      backgroundColor: theme.bg, borderRadius: 20, padding: 28, width: 300, minHeight: 400,
      borderWidth: 1, borderColor: theme.border, alignItems: "center",
    }}>
      <View style={{
        paddingHorizontal: 16, paddingVertical: 4, borderRadius: 20, borderWidth: 1,
        borderColor: theme.border, backgroundColor: "rgba(255,255,255,0.1)", marginBottom: 20,
      }}>
      ...
      <View style={{
        width: 80, height: 80, borderRadius: 40, borderWidth: 2, borderColor: theme.accent,
        backgroundColor: "rgba(255,255,255,0.08)", alignItems: "center", justifyContent: "center", marginBottom: 14,
      }}>
      ...
      {showName && name ? (
        <Text style={{ fontSize: 16, fontWeight: "700", color: theme.text, marginBottom: 4, textAlign: "center" }}>{name}</Text>
      ) : null}

      <Text style={{ fontSize: 11, color: theme.accent, fontWeight: "600", marginBottom: 2 }}>في رحلة العودة إلى الله</Text>
      <Text style={{ fontSize: 10, color: theme.text, opacity: 0.6, marginBottom: 20 }}>{hijriDate}</Text>

      <View style={{
        width: "100%", backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 12,
      ...
      <Text style={{ fontSize: 9, color: theme.text, opacity: 0.5, marginTop: 16 }}>تبت إلى الله توبة نصوحاً</Text>
    </View>
  );
}
...
export default function TawbahCardScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      const [storedName, dayOneDate] = await Promise.all([
        AsyncStorage.getItem("tawbah_card_name"),
        AsyncStorage.getItem("tawbah_day_one_date"),
      ]);
      if (storedName) setName(storedName);
      if (dayOneDate) {
        const diff = Math.floor((Date.now() - Number(dayOneDate)) / 86400000);
        setDayNumber(Math.max(1, diff + 1));
      }
    };
    load();
  }, []);
  ...
  const handleNameChange = async (val: string) => {
    setName(val);
    await AsyncStorage.setItem("tawbah_card_name", val);
  };

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: bg }} edges={["top"]}>
      <PageHeader titleAr="بطاقة توبتي" subtitleAr="اصنع بطاقتك وشاركها" />

      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
        <View style={{ alignItems: "center", marginBottom: 20 }}>
          <CardPreview theme={theme} verse={verse} name={name} showName={showName} dayNumber={dayNumber} hijriDate={hijriDate} />
        </View>

        <View style={{ backgroundColor: cardBg, borderRadius: 16, borderWidth: 1, borderColor, padding: 16, marginBottom: 12 }}>
          <Text style={{ fontSize: 11, fontWeight: "700", color: subColor, marginBottom: 8, textAlign: "right", fontFamily: "IBMPlexSansArabic_700Bold" }}>
          ...
          <View style={{ flexDirection: "row-reverse", gap: 8 }}>
            <TextInput
              ...
              style={{
                flex: 1, backgroundColor: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.04)",
            ...
            <Pressable onPress={() => setShowName(!showName)}
              ...
              {showName ? <Eye size={16} color="#8b5cf6" /> : <EyeOff size={16} color={subColor} />}
            </Pressable>
          </View>
        </View>

        <View style={{ backgroundColor: cardBg, borderRadius: 16, borderWidth: 1, borderColor, padding: 16, marginBottom: 12 }}>
          <Text style={{ fontSize: 11, fontWeight: "700", color: subColor, marginBottom: 10, textAlign: "right", fontFamily: "IBMPlexSansArabic_700Bold" }}>
            اختر التصميم
          </Text>
          <View style={{ flexDirection: "row-reverse", flexWrap: "wrap", gap: 8 }}>
            {THEMES.map((t, i) => (
              <Pressable key={t.id} onPress={() => setThemeIdx(i)}
                style={{
                  paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, backgroundColor: t.bg,
        ...
        <View style={{ backgroundColor: cardBg, borderRadius: 16, borderWidth: 1, borderColor, padding: 16, marginBottom: 16 }}>
          <View style={{ flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <Text style={{ fontSize: 11, fontWeight: "700", color: subColor, fontFamily: "IBMPlexSansArabic_700Bold" }}>الآية</Text>
            <Pressable onPress={() => setVerseIdx((i) => (i + 1) % VERSES.length)}
              style={{ flexDirection: "row-reverse", alignItems: "center", gap: 4 }}>
              <RefreshCw size={12} color="#8b5cf6" />
              <Text style={{ fontSize: 12, color: "#8b5cf6", fontWeight: "700", fontFamily: "IBMPlexSansArabic_700Bold" }}>غيّر الآية</Text>
            </Pressable>
          </View>
          <Text style={{ fontSize: 13, color: textColor, textAlign: "right", lineHeight: 22, fontFamily: "Amiri_400Regular" }} numberOfLines={3}>
            {verse.arabic}
          </Text>
          <Text style={{ fontSize: 11, color: "#8b5cf6", fontWeight: "700", marginTop: 4, textAlign: "right", fontFamily: "IBMPlexSansArabic_700Bold" }}>
            {verse.source}
          </Text>
        </View>
        ...
        <Pressable onPress={handleShare}
          ...
          <Text style={{ fontSize: 16, fontWeight: "700", color: "#fff", fontFamily: "IBMPlexSansArabic_700Bold" }}>
            {shared ? "تمت المشاركة!" : "شارك البطاقة"}
          </Text>
        </Pressable>

        <View style={{ marginTop: 12, padding: 14, backgroundColor: isDark ? "rgba(139,92,246,0.08)" : "rgba(139,92,246,0.06)", borderRadius: 12, borderWidth: 1, borderColor: "rgba(139,92,246,0.2)" }}>
