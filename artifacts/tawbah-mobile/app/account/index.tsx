...
function StatCard({
  ...
  return (
    <View style={{
      ...
      <Text style={{ fontSize: 11, color: isDark ? "#9ca3af" : "#6b7280", textAlign: "center" }}>{label}</Text>
    </View>
  );
}
...
function SettingsRow({
  icon, label, sub, rightEl, onPress, isDark,
}: {
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, alignItems: "flex-end" }}>
        ...
        {sub && <Text style={{ fontSize: 11, color: subColor, marginTop: 1 }}>{sub}</Text>}
      </View>
...
function getQuranDays(): number {
  try {
    return parseInt(localStorage.getItem("quran_streak_days") || "0", 10);
  } catch {
    return 0;
  }
}

function getHabitsDone(): number {
  try {
    const raw = localStorage.getItem("habits_completed_count");
...
export default function AccountScreen() {
  ...
  const [name, setName] = useState(() => {
    try { return localStorage.getItem("user_display_name") || ""; } catch { return ""; }
  });
  ...
  const saveName = () => {
    try { localStorage.setItem("user_display_name", name); } catch {}
    setEditingName(false);
  };

  const currentAccentColor = ACCENT_OPTIONS.find((a) => a.id === accentColor)?.color || "#10b981";
  const dayNum = getDayNumber();
  const quranDays = getQuranDays();
  const habitsDone = getHabitsDone();
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: bg }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
            ...
            {editingName ? (
              <View style={{ flexDirection: "row-reverse", gap: 8, marginBottom: 6 }}>
                <TextInput
                  ...
                  style={{
                    borderWidth: 1, borderColor: currentAccentColor,
                  ...
                  autoFocus
                />
                <Pressable onPress={saveName} style={{ paddingHorizontal: 16, paddingVertical: 8, backgroundColor: currentAccentColor, borderRadius: 10 }}>
                  <Text style={{ color: "#fff", fontWeight: "700", fontSize: 14 }}>حفظ</Text>
                </Pressable>
              </View>
            ) : (
              <Pressable onPress={() => setEditingName(true)}>
                <Text style={{ fontSize: 18, fontWeight: "800", color: textColor, marginBottom: 2 }}>
                  {name || "اضغط لإضافة اسمك"}
                </Text>
              </Pressable>
            )}
            <Text style={{ fontSize: 12, color: subColor }}>اليوم {dayNum} من رحلة التوبة</Text>
          </View>
        ...
        <SectionHeader title="الثيم" isDark={isDark} />
        <View style={{ marginHorizontal: 16, backgroundColor: cardBg, borderRadius: 16, borderWidth: 1, borderColor, paddingVertical: 4 }}>
          {THEME_OPTIONS.map(({ id, label, icon: Icon }) => (
            <SettingsRow
              key={id}
              icon={<Icon size={16} color={theme === id ? currentAccentColor : subColor} />}
              label={label}
              isDark={isDark}
              onPress={() => setTheme(id)}
              rightEl={
                theme === id ? (
                  <View style={{ width: 20, height: 20, borderRadius: 10, backgroundColor: currentAccentColor, alignItems: "center", justifyContent: "center" }}>
          ...
          <View style={{ flexDirection: "row-reverse", gap: 10 }}>
            {ACCENT_OPTIONS.map((opt) => (
              ...
              >
                ...
                <Text style={{ fontSize: 10, color: accentColor === opt.id ? opt.color : subColor, fontWeight: "700" }}>
                  {opt.label}
                </Text>
              </Pressable>
            ))}
        ...
        <SectionHeader title="اللغة" isDark={isDark} />
        <View style={{ marginHorizontal: 16, backgroundColor: cardBg, borderRadius: 16, borderWidth: 1, borderColor, paddingVertical: 4 }}>
          {[
            ...
            <SettingsRow
              key={id}
              icon={<Globe size={16} color={language === id ? currentAccentColor : subColor} />}
              label={label}
              isDark={isDark}
        ...
        <SectionHeader title="التطبيق" isDark={isDark} />
